...
public class InternalInputBuffer extends AbstractInputBuffer<Socket> {
     ...
     * 传统中CR-LF
     * 在 ASCII 的设计里，CR 表示把打印头移动到行首，LF 表示把打印头向下移动一行
     */
    @Override
    public boolean parseRequestLine(boolean useAvailableDataOnly) throws IOException {
        ...
        * */
        while (!space) {
            ...
            // Read new bytes if needed 判断是否过界
            if (pos >= lastValid) {
                if (!fill()){
                    throw new EOFException(sm.getString("iib.eof.error"));
                }
        ...
        //如果包含查询条件 即？ 则 分别设置uriMB和queryMB
        if (questionPos >= 0) {
            //若查询字符串使用汉字 则使用UTF-8编码后每个字节前面加上% 加上% 是因为这是经过了URL编码
        ...
        while (!eol) {
            ...
            if (buf[pos] == Constants.CR) {
                //遇到Constants.CR 代表以及解析到最后了
                end = pos;
            } else if (buf[pos] == Constants.LF) {
                //到 \n才真正结束一行 这里为了处理兼容linux的情况，linux下 \n才是换行符
                if (end == 0){
                    end = pos;
                }
                eol = true;
    ...
    private boolean parseHeader() throws IOException {
        ...
        while (!colon) {
            ...
            if (buf[pos] == Constants.COLON) {
                //遇到: 了则代表头部名字解析完了
                colon = true;
                //把头部名字放到 MimeHeaders的headers里面去。每个都是一个MimeHeaderField对象（包括头部名和头部值）
        ...
        while (validLine) {
            ...
            while (!eol) {
                ...
                if (buf[pos] == Constants.CR) {
                    //不做处理 但是pos会增加，realPos不会增加 下面要是遇到真正的字符的时候，则会覆盖
                    // Skip
                } else if (buf[pos] == Constants.LF) {
                    int iaaa=1;
                    eol = true;
                } else if (buf[pos] == Constants.SP) {
                    //这个地方的赋值是干嘛的？为了跳过中间的Constants.CR？
                    buf[realPos] = buf[pos];
                    realPos++;
                } else {
                    int aa;
        ...
        // Set the header value 解析到结束的时候 再把头部对应的值 塞到对应的地方
