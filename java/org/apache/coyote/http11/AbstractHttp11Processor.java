...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
        ...
        while (!getErrorState().isError() && keepAlive && !comet && !isAsync() &&
            ...
            try {
                ...
                if (endpoint.isPaused()) {
                    ...
                    //这里的request并没有继承 HttpServletRequest
                    request.getMimeHeaders().setLimit(endpoint.getMaxHeaderCount());
                    request.getCookies().setLimit(getMaxCookieCount());
                    // Currently only NIO will ever return false here
                    //调用不同的InputBuffer实现类来 处理头部信息
                ...
                getLog().info(sm.getString("http11processor.header.parse"), e);
                setErrorState(ErrorState.CLOSE_NOW, e);
                break;
            } catch (Throwable t) {
                ...
                // 400 - Bad Request返回400 代表在解析请求行和头部的时候出现问题 一般应该是url出错
            ...
            if (!getErrorState().isError()) {
                ...
                try {
                    //TODO 这里在干嘛 在真正处理request之前先做一些处理 比如头部的进一步解析（）
                    prepareRequest();
                } catch (Throwable t) {
                    ExceptionUtils.handleThrowable(t);
                    getLog().info(sm.getString("http11processor.request.prepare"), t);

                    // 500 - Internal Server Error 这里若抛出异常 则返回500
            ...
            if (!getErrorState().isError()) {
                try {
                    ...
                    //TODO 这里是调用的地方了吧
                    //这里是CoyoteAdapter 无论是apr 还是nio 还是bio 最后都是走到这里
    ...
    protected void prepareRequest() {
        ...
        * */
        ...
        // Check connection header 下面的都是最头部的一些处理
        ...
        if (connectionValueMB != null) {
            ...
            if (findBytes(connectionValueBC, Constants.CLOSE_BYTES) != -1) {
                //如果 是 Connection:close 则keepAlive为false
                keepAlive = false;
            } else if (findBytes(connectionValueBC, Constants.KEEPALIVE_BYTES) != -1) {
                //如果 是 Connection: keep-alive 则keepAlive为false
                keepAlive = true;
            }
        }
        ...
        if (expectMB != null) {
            if (expectMB.indexOfIgnoreCase("100-continue", 0) != -1) {
                //expect:100-continue 头部的处理
                //先尝试向服务器发生一个只包含头部的请求，服务器回送100 状态码后，继续发送带消息体的请求
                //代理服务器对这个头部的处理？
                getInputBuffer().setSwallowInput(false);
                expectation = true;
            } else {
                //若expect 头部的value不为空 ，但是也不是100-continue 则报错SC_EXPECTATION_FAILED 417
