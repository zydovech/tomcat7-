...
public class Catalina {
    ...
    protected Digester createStartDigester() {
        ...
        //为Server添加ObjectCreateRule 规则 该规则用于创建对象
        digester.addObjectCreate("Server",
                                 "org.apache.catalina.core.StandardServer",
                                 "className");
        //为Server添加SetPropertiesRule 规则 该规则用于设置属性。。即为StandardServer添加属性
        digester.addSetProperties("Server");

        //为Server添加 SetNextRule 规则 当遇到结束标签的时候，Catalina调用setServer方法，参数是org.apache.catalina.Server类型的方法
        //即把StandardServer 加入到Catalina中
        ...
        //当遇到Server/GlobalNamingResources结束标签的时候 调用StandardServer的setGlobalNamingResources方法 设置globalNamingResources属性
        digester.addSetNext("Server/GlobalNamingResources",
                            "setGlobalNamingResources",
                            "org.apache.catalina.deploy.NamingResources");

        //创建Server/Listener 指定的className 指定的类。。。Listener标签必须要有className属性
        ...
        //Server/Service/Connector 对应的规则
        ...
        //Server/Service/Connector/Listener 对应的规则
    ...
    public void load() {
        ...
        try {
            try {
                ...
                log.info(sm.getString("catalina.configFail", file), e);
            }
            if (inputStream == null) {
                try {
                    ...
                    log.info(sm.getString("catalina.configFail", getConfigFile()), e);
            ...
            if( inputStream==null ) {
                try {
                    ...
                    log.info(sm.getString("catalina.configFail", "server-embed.xml"), e);
                }
            }
            ...
            try {
                ...
                //在解析之前把Catalina压入栈中。
                digester.push(this);
                //通过Digester解析这个文件，在此过程中会初始化各个组件实例及其依赖关系 最后是用啥SAX
                //调用并解析server.xml文件
        ...
        try {
            // 调用Server的initialize方法，初始化各个组件 最后调用的是StandardServer 的initInternal()
            getServer().init();
        } catch (LifecycleException e) {
